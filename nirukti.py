...
if train_model:
    ...
    model_path = st.text_input("Enter the path to the pre-trained model:", "model.pth")
...
# Generate text
def generate(model, start_string, int_to_vocab, vocab_to_int, device, top_k=5, length=100):
    model.eval()
    input_text = [vocab_to_int[word] for word in start_string.split()]
...
start_string = st.text_input("Ask your question:")

if st.button("Get Answer"):
    generated_text = generate(model, start_string, int_to_vocab, vocab_to_int, device)
    st.write("Answer:")
